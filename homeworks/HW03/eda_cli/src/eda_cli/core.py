...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        # Проверяем: уникальных значений = 1 и есть не-NaN значения
        if col.unique == 1 and col.non_null > 0:
            hasConstant = True
            break
    flags['has_constant_columns'] = hasConstant

    highCardinality = False
    categoricalThreshold = 100
    for col in summary.columns :
        # Проверяем, что колонка категориальная
        if (col.dtype in ['object', 'category']) and (col.unique > categoricalThreshold):
            highCardinality = True
            break
    flags["has_high_cardinality_categoricals"] = highCardinality
